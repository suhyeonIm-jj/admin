...
export default function WorkPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [linksRes, categoriesRes, tagsRes] = await Promise.all([
        ...
        fetch("/api/categories?type=work"),
        fetch("/api/tags"),
      ]);
  ...
  const pinnedLinks = filteredLinks
    .filter((link) => link.isPinned)
    .sort((a, b) => a.order - b.order);
  const frequentLinks = [...filteredLinks]
    .filter((link) => !link.isPinned)
  ...
  // Get unique tags from work links
  const workTags = tags.filter((tag) =>
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <h1 className="text-2xl font-bold">Work Links</h1>
        <button
          onClick={() => {
            setEditingLink(null);
            setIsModalOpen(true);
      ...
      <div>
        ...
        {categories.map((category) => {
          const categoryLinks = filteredLinks.filter(
            (link) => link.category === category.id && !link.isPinned
          );
          return (
            <CategorySection
              key={category.id}
